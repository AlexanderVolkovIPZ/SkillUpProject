...
export default function TaskSubPage({ setValueTab }) {
  ...
  const { data: taskData, isLoading: isLoadingTask } =
    useGetTasksByCourseQuery(id);
  const { data: taskUserData, isLoading: isLoadingUserData } =
    useGetTaskUserQuery();
  const { data: courseUserData, isLoading: isLoadingCourseUserData } =
  ...
  useEffect(() => {
    if (courseUserData && taskUserData) {
      const taskCounts = (taskUserData?.["hydra:member"] ?? []).reduce(
        (counts, task) => {
      ...
      taskCounts.all = (courseUserData["hydra:member"] ?? []).reduce(
        (counter, item) => {
  ...
  return (
    <>
      ...
      {isLoadingTask || isLoadingUserData ? (
          ...
          <CardContent className={s.cardContent}>
            ...
            {taskData
              ...
              ?.map(({ name, createdAt, id, dueDate }) => {
                return (
                  <Accordion className={s.accordion} key={name}>
                    <AccordionSummary
                    ...
                    <AccordionDetails className={s.accordionDetails}>
                      <Box>
                        <Typography>
                          Term:{" "}
                          {dueDate ? dueDate?.date.split(".")[0] : "no term"}
                        </Typography>
                      </Box>
                      <Box className={s.taskStatusesWrapper}>
                        <div>
                          <Typography
                            className={s.countSubmittedTasks}
                            variant="h4"
                          >
                            {taskStatistic[id] ? taskStatistic[id]?.done : 0}
                          </Typography>
                          <Typography variant="title1">Submitted</Typography>
                        </div>
                        <div>
                          <Typography
                            className={s.countAppointedTasks}
                            variant="h4"
                          >
                            {taskStatistic?.all ? taskStatistic?.all : 0}
                          </Typography>
